...
class Board
  ...
  def self.convert_to_xy(coords)
    x, y = coords[0], coords[1..-1]
    [X_AXIS[x.upcase], Y_AXIS[y]]
  end

  def self.convert_to_ln(x, y)
    "#{X_AXIS.key(x)}#{Y_AXIS.key(y)}"
  end

  def self.valid_move?(coords)
  ...
  def hit?(l, n)
    if @grid[n][l] == :~
      @grid[n][l], @display[n][l] = :M, :M
      return false
    elsif @grid[n][l] == :F
      @grid[n][l], @display[n][l] = :X, :X
      return true
    else
      #sets coordinates on grid for player that the board belongs to, and display to be seen by the enemy player
